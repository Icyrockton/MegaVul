...
import io.joern.c2cpg.testfixtures.{DataFlowCodeToCpgSuite, DataFlowTestCpg}
...
class MegaVul_C_TestCpg extends DataFlowTestCpg {
...
class MegaVul_CPP_TestCpg extends DataFlowTestCpg {
...
class MegaVulGraphGenerateForCAndCppTest extends DataFlowCodeToCpgSuite {

  def importCode(inputFilePath: String): Cpg = {
    val content = Files.readString(Paths.get(inputFilePath))
    if (inputFilePath.endsWith(".c")) {
      return new MegaVul_C_TestCpg().moreCode(content)
    }
    else if (inputFilePath.endsWith(".cpp")) {
      return new MegaVul_CPP_TestCpg().moreCode(content)
    }
    else
  ...
  "generateGraph" should {
    ...
    println("MegaVul Generate C/CPP Graph Done!")
  }
}
