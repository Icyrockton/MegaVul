...
module.exports = grammar({
  ...
  rules: {
    ...
    _declaration_modifiers_post: $ => choice(
      $.storage_class_specifier,
      $.linux_specifier,
      $.other_project_specifier,
      $.type_qualifier,
    ...
    _declaration_specifiers: $ => seq(
      ...
      field('type', $._type_specifier),
      repeat($._declaration_modifiers_post),
    ),
    ...
    linux_call_modifier: $ => choice(
      seq('__acquires','(',$.identifier,')'),
      seq('__releases','(',$.identifier,')'),
      seq('__must_hold','(',$.identifier,')'),
    ),
    ...
    other_project_specifier: $ => choice(
      'RAS_VAR_', //freetype
      'RAS_ARG_', //freetype
      'EXEC_OP_', //freetype
      'TSRMLS_DC', //stefanesser/suhosin
      'av_restrict', //stefanesser/suhosin
      '_U_', //stefanesser/suhosin
      'RAS_ARGS', //stefanesser/suhosin
      'UNUSED', //
      'UNUSED_ATTR', //
      'pTHX_', //perl5-dbi/DBD-mysql
      'magick_restrict', //ImageMagick
      'IN', // 01org/opa-fm
      'OUT', // 01org/opa-fm
      'EXEC_OPS', // ArtifexSoftware/ghostpdl
      'CGIFFF', // pts/sam2p
      '__in', // uclouvain/openjpeg
      '_Restrict_', // gnulib
      'RESTRICT', // gnulib
      'OMX_IN', // android
      'SK_RESTRICT', // skia
      '__restrict__', // tensorflow
      'SUB_STATE_RETURN', // openssl
      '_stdcall', // wireshark
      'GLOB_ATTRIBUTE', // glibc
      'MRB_API', // mruby
      'R_API', // radareorg/radare2
    ),

    other_project_post_specifier: $ => choice(
      '_U_', // wireshark
      'UNUSED', // vim
      'TSRMLS_DC', // stefanesser/suhosin
      '_GL_UNUSED', // wget
      '__unused', // android
      'ATTRIBUTE_UNUSED', // binutils-gdb
      'OVS_UNUSED', // openvswitch/ovs
      'GCC_UNUSED', // ThomasDickey/xterm-snapshots
      'HB_UNUSED', //
    ),
    ...
    parameter_declaration: $ => seq(
      ...
      optional(field('declarator', choice(
        seq($._declarator,
            optional($.other_project_post_specifier)
            ),
        $._abstract_declarator
      ))),
    ),
    ...
    do_statement: $ => seq(
      'do',
      field('body', $._statement),
      choice('while','while_each_thread') ,
      field('condition', $.parenthesized_expression),
      ';'
    ),
    ...
    string_literal: $ => seq(
      choice('L"', 'u"', 'U"', 'u8"', '"',
          /KERN_INFO\s*"/,/KERN_DEBUG\s*"/,/KERN_WARNING\s*"/,/KERN_NOTICE\s*"/,
          /KERN_CRIT\s*"/,/KERN_EMERG\s*"/,/KERN_ERR\s*"/,/KERN_CONT\s*"/,/KERN_ALERT\s*"/,
          /XENLOG_G_INFO\s*"/ , /XENLOG_INFO\s*"/ , /XENLOG_WARNING\s*"/ , /XENLOG_ERR\s*"/ , /XENLOG_G_WARNING\s*"/ , /XENLOG_G_ERR\s*"/,
          /LOCK_PREFIX\s*"/
          ),
      repeat(choice(
